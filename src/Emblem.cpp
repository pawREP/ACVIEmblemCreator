...
ErrorOr<std::vector<Emblem>> Emblem::fromJson(const nlohmann::json& json_, uint32_t chromaKey) {
    if(!validateJsonFormat(json_))
    ...
    // Has to be calculated before chromakey or the bb defininf rect might get keyd
    BoundingBox bb = BoundingBox::fromJson(json_);

    nlohmann::json json = nlohmann::json::array();
    if(chromaKey) {
        for(const auto& entry : json_) {
            auto rgba = std::bit_cast<RGBA>(chromaKey);
            if(rgba.r != entry["color"][0] || rgba.g != entry["color"][1] || rgba.b != entry["color"][2])
    ...
    for(int i = 0; i < emblemCount; ++i) {
        ...
        int layerSize = (i != emblemCount - 1) ? perEmblemLayerCountMax : entries.size() % perEmblemLayerCountMax;
